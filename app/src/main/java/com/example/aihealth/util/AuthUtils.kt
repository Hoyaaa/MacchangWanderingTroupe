...
object AuthUtils {
    ...
    fun currentEmail(context: Context, fallbackIntent: Intent? = null, cacheIfFound: Boolean = true): String? {
        val viaIntent = fallbackIntent?.getStringExtra("user_email")
        if (!viaIntent.isNullOrBlank()) { if (cacheIfFound) cacheLastEmail(context, viaIntent); return viaIntent }
        val viaFirebase = FirebaseAuth.getInstance().currentUser?.email
        if (!viaFirebase.isNullOrBlank()) { if (cacheIfFound) cacheLastEmail(context, viaFirebase); return viaFirebase }
        val viaGoogle = GoogleSignIn.getLastSignedInAccount(context)?.email
        if (!viaGoogle.isNullOrBlank()) { if (cacheIfFound) cacheLastEmail(context, viaGoogle); return viaGoogle }
    ...
    fun readLastEmail(context: Context): String? = prefs(context).getString(KEY_LAST_EMAIL, null)
